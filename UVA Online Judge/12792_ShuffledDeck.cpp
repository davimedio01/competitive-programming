...
int main(){
     ...
     while(cin >> num)
     {
          //Aplicando fórmula e conceito acima
          ...
          //Realizando embaralhamento
          while (primeira_pos != 1)
          {
               if (primeira_pos <= num / 2)
                    primeira_pos *= 2; //andar sempre o dobro da posição anterior (ou a própria posição)
               else
                    primeira_pos -= (num + 1 - primeira_pos); //quando ele ultrapassar a metade do total, ele vai
               cont++;
          }
          cout << cont << endl;
     }
